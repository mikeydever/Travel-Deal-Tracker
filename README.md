...
2. Copy the sample environment variables and fill in keys
   ...
   - `ANTHROPIC_API_KEY` – optional Claude provider for Blog Agent drafting
...
- Optional sample data sits in `supabase/seeds/seed_data.sql` to hydrate dashboards locally:
  ...
  ```
- Tables follow `DATA_MODEL.md` (flight_prices, hotel_prices, events, alerts_log, blog_posts) and include JSONB metadata fields so upcoming agents can stash API payload fragments without schema churn.
...
- Blog agent can be run via `POST /api/cron?scope=blog`. It combines internal price deltas + curated external sources, drafts with Claude/OpenAI fallback, and only publishes when quality checks pass.
...
- `/blog`: source-backed daily market updates generated by the Blog Agent.
